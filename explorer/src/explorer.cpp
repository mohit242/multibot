...
#define OPERATE_ON_GLOBAL_MAP                                                  \
    true // global or local costmap as basis for exploration
#define OPERATE_WITH_GOAL_BACKOFF                                              \
    false // navigate to a goal point which is close to (but not exactly at)
...
class Explorer
{
  ...
  public:
          ...
          global_costmap_iteration(0), robot_prefix_empty(false), robot_id(0)
    {
        ...
        nh.param("number_unreachable_for_cluster",
                 number_unreachable_frontiers_for_cluster, 3);
        ...
        std::string service =
            robot_prefix + std::string("/map_merger/logOutput");
        ...
        if (robot_prefix.empty())
        {
            ...
            ROS_INFO("Robot name: %s    robot_id: %d", robot_name.c_str(),
                     robot_id);
        }
        else
        {
        ...
        if (OPERATE_ON_GLOBAL_MAP == true)
        {
            costmap2d_local_size =
                new costmap_2d::Costmap2DROS("local_costmap", tf);
        ...
        else
        {
            ...
            ROS_INFO(
                "Local costmap created ... now performing costmap -> pause");
        ...
        ROS_INFO("Set home point to (%lf,%lf).", robot_home_position_x,
        ...
        exploration->storeVisitedFrontier(robot_home_position_x,
                                          robot_home_position_y, robot_id,
                                          robot_name, -1);
        exploration->storeFrontier(robot_home_position_x, robot_home_position_y,
                                   robot_id, robot_name, -1);

        exploration->setRobotConfig(robot_id, robot_home_position_x,
                                    robot_home_position_y, move_base_frame);
    ...
    void explore()
    {
        ...
        ros::Publisher twi_publisher =
            nh.advertise<geometry_msgs::Twist>("/Rosaria/cmd_vel", 3);
        twi.angular.z = 0.75;
        ...
        while (exploration_finished == false)
        {
            ...
            if (Simulation == false)
            {
                ...
                // ros::Duration(1.0).sleep();
                exploration->initialize_planner(
                    "exploration planner", costmap2d_local, costmap2d_global);
                ...
                else if (frontier_selection == 0)
                {
                    ...
                    while (true)
                    {
                        goal_determined = exploration->determine_goal(
                            2, &final_goal, count, 0, &robot_str);
                        ROS_DEBUG("Goal_determined: %d   counter: %d",
                                  goal_determined, count);
                        if (goal_determined == false)
                        {
                            break;
                        }
                ...
                else if (frontier_selection == 5)
                {
                    ...
                    while (true)
                    {
                        ...
                        if (goal_determined == false)
                        {
                            ROS_INFO("Another cluster is not available, no "
                                     "cluster determined");
                            break;
                        }
                        else
                        {
                            ...
                            {
                                /*
                                 *  TODO ... just to reduce the selection of the
                                 * same
                                 * clusters since no auctioning is implemented
                                 * jet.
                                 */
                                if (robot_id == 1)
                                {
                            ...
                            negotiation = exploration->negotiate_Frontier(
                                ...
                                final_goal.at(2), final_goal.at(3),
                                final_goal.at(4));
                            if (negotiation == true)
                            {
                            ...
                            count++;
                            /*
                             * In order to make one robot wait until the other
                             * finds
                             * another cluster to operate in, one have to fill
                             * the clusters_available_in_pool
                             * vector if count is increased at least once which
                             * determines
                ...
                else if (frontier_selection == 6)
                {
                    ...
                    while (true)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                cluster_element =
                                    int(exploration->clusters.size() * rand() /
                                        (RAND_MAX));
                                ROS_INFO("Random cluster_element: %d  from "
                                         "available %zu clusters",
                                         cluster_element,
                ...
                else if (frontier_selection == 1)
                {
                    ...
                    while (true)
                    {
                        ...
                        if (cluster_element != -1)
                        {
                            ...
                            {
                                for (int i = 0;
                                     i < exploration->clusters.size(); i++)
                                {
                                    if (exploration->clusters.at(i).id ==
                                        cluster_element)
                                    {
                                        if (exploration->clusters.at(i)
                                                .cluster_element.size() > 0)
                                        {
                        ...
                        if (goal_determined == false)
                        {
                            ...
                            bool auctioning = exploration->auctioning(
                                &final_goal, &clusters_available_in_pool,
                                &robot_str);
                            if (auctioning == true)
                            {
                            ...
                            {
                                if (exploration->clusters.size() >
                                    0) // clusters_available_in_pool.size() > 0)
                                {
                ...
                if (navigate_to_goal == true && goal_determined == true)
                {
                    ...
                    exploration->storeVisitedFrontier(
                        final_goal.at(0), final_goal.at(1), final_goal.at(2),
    ...
    void map_info()
    {
        ...
        while (ros::ok() && exploration_finished != true)
        {
            ...
            double exploration_travel_path_global =
                (double)exploration->exploration_travel_path_global * 0.02;
            double exploration_travel_path_average = 0;
            if (counter != 0)
            {
                exploration_travel_path_average =
                    (exploration->exploration_travel_path_global) / counter;
            }
            ...
            fs_csv.open(csv_file.c_str(), std::fstream::in | std::fstream::app |
                                              std::fstream::out);
    ...
    int local_costmap_size()
    {
        if (OPERATE_ON_GLOBAL_MAP == false)
        {
            ...
            int num_map_cells_ =
                costmap2d_local->getCostmap()->getSizeInCellsX() *
        ...
        else
        {
            occupancy_grid_local =
                costmap2d_local_size->getCostmap()->getCharMap();
            int num_map_cells_ =
                costmap2d_local_size->getCostmap()->getSizeInCellsX() *
    ...
    void initLogPath()
    {
        ...
        if (!boost::filesystem::exists(boost_log_path))
        {
            ...
            {
                ROS_INFO("Successfully created directory %s.",
                         log_path.c_str());
            }
        }
    ...
    void save_progress(bool final = false)
    {
        ...
        double exploration_travel_path =
            (double)exploration->exploration_travel_path_global *
            costmap_resolution;
        ...
        double efficiency_value =
            (exploration_time) / (number_of_robots * navigation_goals_required);
        ...
        fs << "required_goals       			= " << navigation_goals_required
           << std::endl;
        fs << "unreachable_goals                            = "
           << exploration->unreachable_frontiers.size() << std::endl;
        fs << "travel_path_overall  			= " << exploration_travel_path
           << std::endl;
        fs << "number_of_completed_auctions                 = "
           << exploration->number_of_completed_auctions << std::endl;
        fs << "number_of_uncompleted_auctions               = "
           << exploration->number_of_uncompleted_auctions << std::endl;
        fs << "frontier_selection_strategy                  = "
           << frontier_selection << std::endl;
        fs << "costmap_size                                 = " << costmap_width
           << std::endl;
        fs << "global costmap iterations                    = "
           << global_costmap_iteration << std::endl;
        ...
        fs << "explorer_local_costmap_height 		= " << param_int
           << std::endl;
        ...
        ros::param::get(param, param_double);
        fs << "move_base_local_costmap_height 		= " << param_double
           << std::endl;

        param = robot_prefix + "/move_base/local_costmap/width";
        ros::param::get(param, param_double);
        fs << "move_base_local_costmap_width 		= " << param_double
           << std::endl;
        ...
        nh.getParam("/global_costmap/obstacle_layer/raytrace_range",
                    param_double);
        ros::param::get(param, param_double);
        ...
        //    ros::param::get(param,param_double);
        nh.getParam("/global_costmap/obstacle_layer/obstacle_range",
                    param_double);
    ...
    void exploration_has_finished()
    {
        ...
        for (int i = 0; i < map_progress_during_exploration.size(); i++)
        {
            ROS_INFO("map progress: %f",
                     (map_progress_during_exploration.at(i).global_freespace /
                      size_global_map) *
                         100);
        }
                                ...
                                std::vector<std::string> *robot_str)
    {
        ...
        //    			    ros::Duration(1.0).sleep();
        exploration->initialize_planner("exploration planner", costmap2d_global,
                                        costmap2d_global);
        exploration->findFrontiers();
        //            exploration->printFrontiers();
        ...
        if (frontier_selection < 5 && frontier_selection != 1)
        {
            ...
            while (true)
            {
                ...
                else
                {
                    ...
                    negotiation = exploration->negotiate_Frontier(
                        global_goal->at(0), global_goal->at(1),
        ...
        else if (frontier_selection == 1 || frontier_selection == 6 ||
                 frontier_selection == 5)
        {
            ...
            while (true)
            {
                ...
                {
                    ROS_INFO("Still some goals in current cluster, finish this "
                             "job first");
                    break;
                }
            }
    ...
    bool navigate(std::vector<double> goal)
    {
        ...
        if (goal_determined == true)
        {
            ...
            counter++;
            ROS_INFO("GOAL %d:  x: %f      y: %f", counter, goal.at(0),
                     goal.at(1));
        ...
        else
        {
            ...
            //                        counter_waiting_for_clusters);
            ROS_INFO(
                "In navigation .... cluster_available: %zu     counter: %d",
                exploration->clusters.size(), counter_waiting_for_clusters);
            //                        if(clusters_available_in_pool.size() <= 0
            //                        || counter_waiting_for_clusters > 10)
            //                        //(rotation_counter >= 2)
            if (exploration->clusters.size() == 0 ||
                counter_waiting_for_clusters > 10) //(rotation_counter >= 2)
            {
                ...
                std::vector<std::string> robot_str;
                bool global_costmap_goal =
                    iterate_global_costmap(&global_goal, &robot_str);

                if (global_costmap_goal == false)
                {
                    //                                global_iterations_counter++;
                    //                                if(global_iterations_counter
                    //                                >= 5)
                    //                                {
                    counter++;
                    ROS_INFO("GOAL %d: BACK TO HOME   x: %f    y: %f", counter,
                             home_point_x, home_point_y);
                ...
                {
                    counter++;
                    ROS_INFO("GOAL %d:  x: %f      y: %f", counter,
                             global_goal.at(0), global_goal.at(1));
                    std::vector<double> backoffGoal;
                    bool backoff_sucessfull = exploration->smartGoalBackoff(
                        global_goal.at(0), global_goal.at(1), costmap2d_global,
                        &backoffGoal);

                    if (backoff_sucessfull == true)
                    {
                        ROS_DEBUG("doing navigation to back-off goal");
                        visualize_goal_point(backoffGoal.at(0),
                                             backoffGoal.at(1));
                        completed_navigation = move_robot(
                            counter, backoffGoal.at(0), backoffGoal.at(1));
                        rotation_counter = 0;
                    ...
                    {
                        ROS_WARN("Navigation to global costmap back-off goal "
                                 "not possible");
    ...
    void visualize_goal_point(double x, double y)
    {
        ...
        pub_Point = nh_Point.advertise<geometry_msgs::PointStamped>("goalPoint",
                                                                    100, true);
    ...
    void visualize_home_point()
    {
        ...
        pub_home_Point =
            nh.advertise<geometry_msgs::PointStamped>("homePoint", 100, true);
    ...
    bool move_robot(int seq, double position_x, double position_y)
    {
        ...
        goal_msgs.target_pose.pose.position.x =
            position_x; // - robotPose.getOrigin().getX();
                        // //goals[0].pose.position.x;
        goal_msgs.target_pose.pose.position.y =
            position_y; // - robotPose.getOrigin().getY();
        goal_msgs.target_pose.pose.position.z = 0;
        goal_msgs.target_pose.pose.orientation.x =
            0; // sin(angle/2); // goals[0].pose.orientation.x;
        ...
        // ac.waitForResult(ros::Duration(20)); EDIT Peter: Test if it also
        // works with smaller value!
        ...
        while (ac.getState() != actionlib::SimpleClientGoalState::SUCCEEDED)
        {
            if (ac.getState() == actionlib::SimpleClientGoalState::ABORTED)
            {
                ...
                exploration->next_auction_position_x =
                    robotPose.getOrigin().getX();
                exploration->next_auction_position_y =
                    robotPose.getOrigin().getY();
                return false;
            }
            /*
             ...
             if(ac.getState() ==
             actionlib::SimpleClientGoalState::PREEMPTED){ROS_ERROR("PREEMPTED");}
             if(ac.getState() ==
             actionlib::SimpleClientGoalState::ABORTED){ROS_ERROR("ABORTED");}
             if(ac.getState() ==
             actionlib::SimpleClientGoalState::REJECTED){ROS_ERROR("REJECTED");}
             if(ac.getState() ==
             actionlib::SimpleClientGoalState::RECALLED){ROS_ERROR("RECALLED");}
             */
        }
    ...
    bool target_reached(void)
    {
        ...
        sub_move_base = nh_sub_move_base.subscribe(
            "feedback", 1000, &Explorer::feedbackCallback, this);
  ...
  public:
    ...
    int frontier_selection, costmap_width, global_costmap_iteration,
        number_unreachable_frontiers_for_cluster;
    ...
    std::string
        robot_prefix; /// The prefix for the robot when used in simulation
...
int main(int argc, char **argv)
{
    ...
    //        boost::thread
    //        thr_frontiers(boost::bind(&SimpleNavigation::frontiers, &simple));
    /*
