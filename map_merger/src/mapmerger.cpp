...
{    
	...
	nodeHandle->param<std::string>("static_mc_group_name", mc_group, "mc_default");
...
void MapMerger::waitForRobotInformation()
{
    ...
    {
        ROS_INFO("%zu robots are already in the network, adding them,my name:%s",getNeighbors.response.neigbors.size(),
                 robot_name.c_str());
...
void MapMerger::callback_control(const adhoc_communication::MmControlConstPtr &msg)
{
    ...
    if(tmp.update_numbers.size() == 0)
    {
        ...
        ROS_INFO("Sended map after beeing asked to %s,containd %zu updates,update_seq:%i",tmp.src_robot.c_str(),containedUpdates->size(),update_seq);
...
void MapMerger::callback_global_pub(const ros::TimerEvent &e)
{
   // sendMetaData(5);
    ...
    {
        ROS_DEBUG("Merging other maps into global map in callback_global_pub,sizeTransforms:%zu,sizeRobots:%zu",transforms->size(),robots->size());
        for(int i = 0; i < transforms->size(); i++)
        {
            ...
            ROS_DEBUG("In mergeMaps MapData:%i,Transform:%i|size MapData:%zu,size Tranform:%zu",
                      i,
    ...
    ROS_DEBUG("Publishing global_map, contains data of local map and %zu other maps| i am having %zu other maps",
              transforms->size(),
              map_data->size());
...
void MapMerger::callback_map_other(const adhoc_communication::MmMapUpdateConstPtr &msg)
{
    ...
    if(index_robots == -1)
    {
        ROS_WARN("Got map with a not valid frame_id:%s| index_robot == -1| size of data:%zu",toInsert->header.frame_id.c_str(),toInsert->data.size());
        return;
    }
    else
    {
        ...
        ROS_DEBUG("GOT NOT LOCAL MAP,Process it, data size:%zu",toInsert->data.size());
        ...
        if(updateMan->isUpdatesMissing(index_robots))
        {
            ...
            ROS_DEBUG("%zu updates are missing,asking for them",listOfMissedUpdates->size());
...
nav_msgs::OccupancyGrid* MapMerger::getMapPart(int map_index, int start_x, int start_y, int width, int height)
{
    ...
    int sizePart = (start_x + height)*(start_y+ width);
     if(sizePart > tmp->data.size())
     {
         ...
         ROS_ERROR("Part is too large \n\t\t\t\tsizePart:%i\ttmp size:%zu\twidth:%i\theight:%i",
...
void MapMerger::sendMapOverNetwork(string destination, std::vector<int>* containedUpdates, int start_row, int start_collumn, int end_row, int end_collumn)
{
    //ros::nodeHandle nodeHandle ("~");
	...
	ROS_DEBUG("Send map using:%s \n\t\t\t\t the normal way min_x:%i|max_x:%i|min_y:%i|max_y:%i",service.c_str(),start_row,end_row,start_collumn,end_collumn);
	/**
	  todo:
	...
	*/
	...
	{
		for(int collumn = start_collumn; collumn < end_collumn; collumn+=size)
		{
			...
			nav_msgs::OccupancyGrid * t = this->getMapPart(-1,row,collumn,width,height);
			...
			*/
			...
			ROS_DEBUG("Send Map to %s(if nothing -> broadcast), frame id:%s| topic:[%s]|data<_size:%zu",
			...
			if (client.call(exchange))
			{
				if (exchange.response.status)
				{
					if (destination == "")
					{
						ROS_DEBUG("Sended Map to: all,topic:%s|r:%i;c:%i",exchange.request.topic.c_str(), row, collumn);
					}
					else
					{
						ROS_DEBUG("Sended Map to: %s,topic:%s|r:%i;c:%i",exchange.request.dst_robot.c_str(), exchange.request.topic.c_str(), row, collumn);
					}
				}
				else
				{
					ROS_WARN("Destination host unreachable [%s](if nothing -> broadcast)",hostname.c_str());
				}
			}
			else
			{
